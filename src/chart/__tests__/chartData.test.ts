...
import type { ScenarioWhatIfState, StochasticProjectionResult } from "@/lib/projection";
...
describe("buildAccountDiagnosticChartData", () => {
  it("returns per-account balances and deterministic net worth when no stochastic data is provided", () => {
    ...
    const result = projectScenarioPack(pack!, PROJECTION_SETTINGS, EMPTY_WHAT_IF);
  ...
  it("merges stochastic band data into rows when stochastic result is provided", () => {
    ...
    const stochasticResult = stochasticProject(pack!, PROJECTION_SETTINGS, EMPTY_WHAT_IF, {
      runCount: 50,
      seed: 42,
    });

    const data = buildAccountDiagnosticChartData(pack!, stochasticResult.deterministic, stochasticResult);
      ...
      bands: [
        {
          date: "9999-01-01",
          isHistorical: false,
          netWorth: { p10: 400_000, p25: 450_000, p50: 500_000, p75: 550_000, p90: 600_000 },
        },
      ],
      milestones: {
        ...
        finalNetWorthPercentiles: { p10: 300_000, p25: 400_000, p50: 500_000, p75: 600_000, p90: 700_000 },
      },
    };
