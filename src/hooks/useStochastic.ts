...
export function useStochastic(
  ...
  const [state, setState] = useState<ProjectionHookState<StochasticProjectionResult>>({
    result: null,
    runtimeError: null,
    isRunning: false,
    progress: null,
  ...
  useEffect(() => {
    ...
    engine
      ...
      .catch((err: unknown) => {
        ...
        setState((s) => ({
          ...s,
          runtimeError: err instanceof Error ? err.message : "Stochastic simulation failed.",
          isRunning: false,
          progress: null,
        }));
      });
