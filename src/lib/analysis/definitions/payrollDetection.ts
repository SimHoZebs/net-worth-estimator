...
import type { TransactionClassificationResult } from "./transactionClassification";
...
export const payrollDetectionAnalysis: AnalysisDefinition<
...
> = {
	...
	run({ input }) {
		...
		for (const classified of input.transactions) {
			const { transaction, classification } = classified;
			if (transaction.amount === null || transaction.amount <= 0) continue;
			if (classification.type !== "payroll") continue;
			const explicit = classification.evidence.some(
				(item) => item.code === "source.transaction-type",
			);
			const hasLanguage = classification.evidence.some(
				(item) => item.code === "payroll.language",
			);
			const identity = classification.payerIdentity;
			...
			const group = grouped.get(key) ?? {
				...
				payerLabel: classification.payerLabel,
				transactions: [],
				evidence: [],
			};
