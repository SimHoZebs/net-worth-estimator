...
function detectPayrollFromPostings(postings: PostingObservation[]) {
	return detectPayroll(classifyPostings({ postings }));
}

describe("payroll detection analysis", () => {
	it("detects recurring payroll-language deposits", () => {
		const result = detectPayrollFromPostings([
	...
	it("uses payroll labels and excludes non-payroll posting labels", () => {
		const result = detectPayrollFromPostings([
	...
	it("keeps identical payer names separated by account", () => {
		const result = detectPayrollFromPostings([
	...
	it("does not merge employers behind a generic counterparty label", () => {
		const result = detectPayrollFromPostings([
	...
	it("does not merge identity-less payroll postings", () => {
		const result = detectPayrollFromPostings([
	...
	it("does not create an identity from punctuation-only payer text", () => {
		const result = detectPayrollFromPostings([
	...
	it("marks cadence-breaking gaps and duplicate dates as non-recurring", () => {
		const result = detectPayrollFromPostings([
			posting("p1", "2026-01-02"),
			posting("p2", "2026-01-16"),
			posting("p3", "2026-01-30"),
		...
		const duplicateDateResult = detectPayrollFromPostings([
