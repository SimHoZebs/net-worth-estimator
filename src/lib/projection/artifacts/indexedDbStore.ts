	...
	implements ProjectionArtifactStore<TPayload>
{
	...
	async get(
		...
		traceProjectionCache("idb.get.start", { key });
		try {
			...
			if (record !== undefined && !isStoredArtifact(record)) {
				await this.deleteIfMalformed(database, key);
				traceProjectionCache("idb.get.complete", { key, hit: false });
				return undefined;
			}
			const envelope = readEnvelope(record);
			traceProjectionCache("idb.get.complete", {
				key,
				hit: envelope !== undefined,
			});
			return envelope;
		} catch (error) {
			traceProjectionCache("idb.get.error", {
				key,
	...
	async delete(key: string): Promise<void> {
		assertArtifactKey(key);
		traceProjectionCache("idb.delete.start", { key });
		try {
			const database = await this.openDatabase();
			const transaction = database.transaction(
				PROJECTION_ARTIFACT_STORE_NAME,
			...
			traceProjectionCache("idb.delete.complete", { key });
		} catch (error) {
			traceProjectionCache("idb.delete.error", {
				key,
				...projectionCacheErrorDetails(error),
			});
			throw error;
	...
	async putIfAbsent(
		...
		traceProjectionCache("idb.put.start", { key });
		try {
			const database = await this.openDatabase();
			const winner = await this.addOrReadWinner(database, key, envelope);
			await this.prune(database).catch((error: unknown) => {
				traceProjectionCache("idb.prune.error", {
					key,
					...projectionCacheErrorDetails(error),
				});
			});
			traceProjectionCache("idb.put.complete", { key });
			return winner;
		} catch (error) {
			traceProjectionCache("idb.put.error", {
				key,
				...projectionCacheErrorDetails(error),
			});
			throw error;
	...
	private openDatabase(): Promise<IDBDatabase> {
		...
		const opening = new Promise<IDBDatabase>((resolve, reject) => {
			...
			request.onsuccess = () => {
				...
				traceProjectionCache("idb.open.complete", {
					databaseName: this.databaseName,
			...
			request.onblocked = () => {
				const error = new Error(
					"Opening the projection artifact database was blocked.",
				);
				traceProjectionCache("idb.open.blocked", {
					databaseName: this.databaseName,
					...projectionCacheErrorDetails(error),
				});
				reject(error);
	...
	private deleteIfMalformed(database: IDBDatabase, key: string): Promise<void> {
		const transaction = database.transaction(
			PROJECTION_ARTIFACT_STORE_NAME,
			"readwrite",
		);
		...
		>;
		request.onsuccess = () => {
			if (request.result !== undefined && !isStoredArtifact(request.result)) {
				store.delete(key);
			}
		};
	...
	private addOrReadWinner(
		...
		return new Promise((resolve, reject) => {
			...
			let winnerSource: "new" | "existing" | undefined;
			...
			transaction.oncomplete = () => {
				...
				traceProjectionCache("idb.put.winner", {
					key,
					source: winnerSource,
				});
				resolve(winner);
