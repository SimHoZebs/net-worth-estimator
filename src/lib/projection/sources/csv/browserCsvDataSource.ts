...
export function createBrowserCsvDataSource(options: BrowserCsvDataSourceOptions = {}): DataSource {
  ...
  return {
    ...
    save: storage
      ? {
          ...
          description: "Stores the edited scenario in local storage for this browser and device.",
          run: async (pack: ScenarioPack): Promise<ScenarioParseResult> => {
            const savedPack = { ...pack, sourcePath: BROWSER_STORAGE_SOURCE_PATH };
    ...
    reset: storage
      ? {
          ...
          description: "Deletes the browser-local scenario and reloads the deployed /scenario CSV files.",
