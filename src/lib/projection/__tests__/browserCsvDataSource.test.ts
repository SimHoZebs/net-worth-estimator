...
function createCsvFetch() {
  return vi.fn(async (input: RequestInfo | URL) => {
    ...
    const body = fileName === "accounts.csv"
...
describe("createBrowserCsvDataSource", () => {
  ...
  it("loads the saved browser pack before fetching bundled CSV files", async () => {
    ...
    const storage = createMemoryStorage({ [storageKey]: JSON.stringify(savedPack) });
    const fetchImpl = createCsvFetch();
    const dataSource = createBrowserCsvDataSource({ fetchImpl, storage, storageKey });
  ...
  it("saves to browser storage and resets back to bundled CSV files", async () => {
    ...
    const saved = JSON.parse(storage.getItem(storageKey)!) as { sourcePath: string };
