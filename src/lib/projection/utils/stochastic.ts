...
export function sampleLogNormal(expectedReturn: number, volatility: number): number {
  if (volatility <= 0) {
  ...
  const standardNormal = Math.sqrt(-2 * Math.log(Math.max(u, 1e-10))) * Math.cos(2 * Math.PI * v);
