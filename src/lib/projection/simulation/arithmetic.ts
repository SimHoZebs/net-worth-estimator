...
class Lexer {
	...
	next(): Token {
		...
		switch (ch) {
			case "+":
				return { kind: "plus", lexeme: "+", offset: start };
			case "-":
				return { kind: "minus", lexeme: "-", offset: start };
			case "*":
				return { kind: "star", lexeme: "*", offset: start };
			case "/":
				return { kind: "slash", lexeme: "/", offset: start };
			case "(":
				return { kind: "lparen", lexeme: "(", offset: start };
			case ")":
				return { kind: "rparen", lexeme: ")", offset: start };
			default:
			...
			node =
				op.kind === "star"
					...
					: (ctx) => {
							...
							if (divisor === 0) {
								throw new ArithmeticEvalError("Division by zero");
							}
...
export class ArithmeticEvalError extends Error {
