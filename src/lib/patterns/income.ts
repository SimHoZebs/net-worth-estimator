...
export function generateIncomePattern(
  ...
  if (!startDate || !/^\d{4}-\d{2}-\d{2}$/.test(startDate)) errors.push("Start date must be YYYY-MM-DD.");
  if (grossMonthlyIncome <= 0) errors.push("Gross monthly income must be positive.");
  if (taxRate < 0 || taxRate > 1) errors.push("Tax rate must be between 0 and 1.");
  if (k401ContributionRate < 0 || k401ContributionRate > 1) errors.push("401(k) contribution rate must be between 0 and 1.");
  if (k401EmployerMatchRate < 0 || k401EmployerMatchRate > 1) errors.push("Employer match rate must be between 0 and 1.");
  if (autoInvestRate < 0 || autoInvestRate > 1) errors.push("Auto-invest rate must be between 0 and 1.");
  ...
  const brokerageAutoId = uniqueId(`${stem}_brokerage_auto`, existingPostingSet);

  const accounts: Account[] = [];
  for (const [id, { color }] of Object.entries(DEFAULT_ACCOUNTS)) {
    if (!existingAccountSet.has(id)) {
      accounts.push(makeAccount(id, id.charAt(0).toUpperCase() + id.slice(1), color));
    }
  }
