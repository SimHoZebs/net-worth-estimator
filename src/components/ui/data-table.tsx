...
import { integer, decimal, currency } from "@/lib/format";
    ...
    <Table>
      ...
      <TableBody>
        {rows.length > 0 ? rows.map((row, ri) => (
          ...
          <TableRow>
            <TableCell colSpan={columns.length} className="py-6 text-center text-slate-500 dark:text-slate-400">{emptyText}</TableCell>
          </TableRow>
        )}
      </TableBody>
    </Table>
