...
import { CheckboxField, Field, NullableMonthField, NumberField, PercentField, SelectField, TextField, type SelectOption } from "./shared";

function scenarioHasModuleType(moduleTypes: ScenarioModule["type"][], type: ScenarioModule["type"]): boolean {
  return moduleTypes.includes(type);
}

const emptyModuleTypes: ScenarioModule["type"][] = [];
...
const ModuleCard = memo(function ModuleCard({
  ...
  updateModule: (moduleIndex: number, updater: (currentModule: ScenarioModule) => ScenarioModule) => void;
}) {
  const { control, setValue } = useFormContext<ScenarioDefinition>();
  const module = useWatch({
    control,
    name: `modules.${moduleIndex}` as const,
  }) as ScenarioModule | undefined;

  if (!module) return null;

  return (
    <Card className="min-w-[360px] max-w-[480px] shrink-0 rounded-[1.75rem] border-slate-200 bg-white">
                  ...
                  <NativeSelect
                    ...
                    onChange={(event) => {
                      ...
                      updateModule(moduleIndex, (currentModule) => {
                        if (currentModule.type !== "recurringFlow") return currentModule;
            ...
            <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
              <div className="flex items-center justify-between gap-4">
                ...
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => updateModule(moduleIndex, (currentModule) => {
                    if (currentModule.type !== "equityGrantSeries") return currentModule;
                    return {
                      ...currentModule,
...
export function ModulesEditor() {
  ...
  const moduleTypes = (useWatch({
    control,
    name: fields.map((_, moduleIndex) => `modules.${moduleIndex}.type` as const),
  }) ?? emptyModuleTypes) as ScenarioModule["type"][];
  ...
  const updateModule = (moduleIndex: number, updater: (currentModule: ScenarioModule) => ScenarioModule) => {
    const currentModule = getValues(`modules.${moduleIndex}` as const);
    if (!currentModule) return;

    setValue(`modules.${moduleIndex}` as const, updater(currentModule), {
      shouldDirty: true,
      shouldTouch: true,
      shouldValidate: true,
    });
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {BUILT_IN_MODULE_ORDER.map((type) => {
            const isDisabled = isSingletonBuiltInModuleType(type) && scenarioHasModuleType(moduleTypes, type);
            ...
            return (
              <Button
                ...
                disabled={isDisabled}
                onClick={() => {
                  const nextModules = addModuleByType(getValues(), type).modules;
