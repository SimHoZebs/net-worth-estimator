    ...
    <Alert className="space-y-3 rounded-[1.6rem] border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950 text-amber-950 dark:text-amber-200">
      ...
      <div className="space-y-2">
        {issues.map((issue, index) => (
          <div key={`${issue.code}-${index}`} className={`rounded-2xl p-3 ${issue.severity === "error" ? "bg-rose-100 dark:bg-rose-900/30 text-rose-950 dark:text-rose-200" : "bg-amber-100 dark:bg-amber-900/30 text-amber-950 dark:text-amber-200"}`}>
