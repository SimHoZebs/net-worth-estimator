      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="rounded-xl border border-slate-100 bg-slate-50 px-2 py-3">
            <div className="text-lg font-semibold text-slate-900">{enabledAccounts}</div>
            <div className="text-[0.65rem] uppercase tracking-[0.14em] text-slate-400">Accounts</div>
          </div>
          <div className="rounded-xl border border-slate-100 bg-slate-50 px-2 py-3">
            <div className="text-lg font-semibold text-slate-900">{enabledPostings}</div>
            <div className="text-[0.65rem] uppercase tracking-[0.14em] text-slate-400">Flows</div>
          </div>
          <div className="rounded-xl border border-slate-100 bg-slate-50 px-2 py-3">
            <div className="text-lg font-semibold text-slate-900">{pack.checkpoints.length}</div>
            <div className="text-[0.65rem] uppercase tracking-[0.14em] text-slate-400">History</div>
          </div>
        </div>

        <details className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
          <summary className="cursor-pointer select-none text-xs font-medium uppercase tracking-[0.16em] text-slate-500">
            Annual rates
          </summary>
          {annualRatePostings.length > 0 ? (
            <div className="mt-3 space-y-2 text-sm">
              {annualRatePostings.map((posting) => (
                <div key={posting.id} className="grid grid-cols-[1fr_auto] gap-3">
                  <span className="text-slate-700">{posting.label}</span>
            ...
            <div className="mt-2 text-sm text-slate-400">No annual rates configured on enabled transactions.</div>
          )}
        </details>
