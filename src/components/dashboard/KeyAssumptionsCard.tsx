      ...
      <CardContent>
        <div className="mb-5 grid gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-4">
            <div className="flex items-center justify-between">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Projection horizon</div>
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-4">
            ...
            <div className="mt-2 text-xl font-semibold text-slate-900 dark:text-slate-100">{activeOverrideCount === 0 ? "None" : String(activeOverrideCount)}</div>
            <div className="mt-1 text-xs text-slate-400 dark:text-slate-500">{activeOverrideCount === 0 ? "Baseline only" : "Temporary scenario changes"}</div>
          </div>
        </div>
        ...
        <div className="mt-4 border-t border-slate-100 dark:border-slate-700 pt-4">
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-600 dark:text-slate-400">
            <span><span className="font-medium text-slate-900 dark:text-slate-100">{pack.accounts.filter((a) => a.enabled).length}</span> accounts tracked</span>
            <span><span className="font-medium text-slate-900 dark:text-slate-100">{pack.postings.filter((p) => p.enabled).length}</span> scheduled transactions</span>
            <span><span className="font-medium text-slate-900 dark:text-slate-100">{pack.checkpoints.length}</span> balance history points</span>
          </div>
        </div>

        <div className="mt-4 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-4 py-3">
          <div className="text-xs font-medium tracking-wide text-slate-500 dark:text-slate-400">Annual rates</div>
          {pack.postings.filter(p => p.enabled && p.annualRate > 0).length > 0 ? (
            <div className="mt-2 grid grid-cols-[auto_1fr_auto] gap-x-6 gap-y-1 text-sm">
              {pack.postings.filter((p) => p.enabled && p.annualRate > 0).map((p) => (
                <div key={p.id} className="contents">
                  ...
                  <span className="text-slate-400 dark:text-slate-500 italic">{p.annualGrowthRate > 0 ? `${(p.annualRate * 100).toFixed(1)}%, growing ${(p.annualGrowthRate * 100).toFixed(1)}%/yr` : `${(p.annualRate * 100).toFixed(1)}%`}</span>
            ...
            <div className="mt-1 text-sm text-slate-400 dark:text-slate-500">No annual rates configured on enabled transactions.</div>
          )}
        </div>
        ...
        <div className="mt-4 space-y-2 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-4 py-3">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Model assumptions</div>
