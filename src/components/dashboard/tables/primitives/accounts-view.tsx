...
export function ReadOnlyAccountsView({
	accounts,
	accountRules,
	accountSummaries,
	currentNetWorth,
	...
	const { search, setSearch, visible } = useSearchFilter(
		rows,
		matchesAccountRow,
	);
	const assetRows = visible.filter(
		(row) => row.balance !== null && row.balance >= 0,
	);
	const liabilityRows = visible.filter(
		(row) => row.balance !== null && row.balance < 0,
	);
	const unavailableRows = visible.filter((row) => row.balance === null);
