...
export function useDashboardDerivedValues(result: ProjectionResult, pack: ScenarioPack): DashboardDerivedValues {
  return useMemo(() => {
    const firstProjectedRow = result.timeline.rows.find((row) => !row.isHistorical) ?? null;
    const firstShortfallRow = result.timeline.rows.find((row) => !row.isHistorical && row.clampedPostingShortfallAmount > 0) ?? null;
    ...
    const blockerValue = biggestShortfallPosting?.label ?? "No constraint showing";
