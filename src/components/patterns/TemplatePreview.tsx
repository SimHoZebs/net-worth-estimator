    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-4 space-y-3">
      ...
      {newAccounts.length > 0 && (
        <div className="space-y-1">
          <span className="text-xs text-slate-500 dark:text-slate-400">New accounts:</span>
      ...
      <div className="space-y-1.5">
        <span className="text-xs text-slate-500 dark:text-slate-400">Postings:</span>
        <div className="space-y-1">
          {postingDescriptions.map((p) => (
            ...
            >
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400">{p.route}</span>
              <code className="text-xs text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700/80 rounded px-1.5 py-0.5">
                {p.arithmetic}
              </code>
              {p.cap && <span className="text-xs text-slate-400 dark:text-slate-500">{p.cap}</span>}
            </div>
          ))}
        </div>
      </div>
