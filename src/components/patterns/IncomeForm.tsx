...
const presets = [
  ...
  { label: "24% bracket", taxRate: 24, k401: 6, match: 50, cap: 23000, auto: 15 },
  { label: "32% bracket", taxRate: 32, k401: 8, match: 3, cap: 22500, auto: 20 },
  { label: "No 401k / low tax", taxRate: 12, k401: 0, match: 0, cap: 0, auto: 25 },
];

export function IncomeForm({ value, onChange }: IncomeFormProps) {
  const update = (patch: Partial<IncomeTemplateInput>) => onChange({ ...value, ...patch });
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => update({ grossMonthlyIncome: Number(e.target.value) })}
            placeholder="10000"
          />
        </div>
        <div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => update({ k401ContributionRate: Number(e.target.value) / 100 })}
            placeholder="4"
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) => update({ k401EmployerMatchRate: Number(e.target.value) / 100 })}
            placeholder="50"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => update({ autoInvestRate: Number(e.target.value) / 100 })}
            placeholder="10"
          />
        </div>
      </div>
