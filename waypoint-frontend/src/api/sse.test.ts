...
const progress = { phase: 'stochastic-runs', completedRuns: 2, totalRuns: 4, fraction: 0.5, evaluationWorkloads: [] } as const;
const result = { config: { runCount: 4, seed: 7 }, bands: [], milestones: { finalNetWorthPercentiles: { p10: 1, p25: 2, p50: 3, p75: 4, p90: 5 } }, evaluations: { financialIndependence: [], netWorthThreshold: [], postingFulfillment: [] } } as const;

describe('incremental POST-SSE parser', () => {
  it('handles split blocks, retry hints, comments, and typed callbacks', () => {
    const callbacks = { onRetry: vi.fn(), onComment: vi.fn(), onProgress: vi.fn(), onPartial: vi.fn(), onResult: vi.fn(), onError: vi.fn() };
    ...
    const third = new TextEncoder().encode(`event: partial\r\ndata: ${JSON.stringify({ progress, partial: result })}\r\n\r\n`);
    const fourth = new TextEncoder().encode(`event: result\ndata: ${JSON.stringify({ result })}\n\nevent: error\ndata: {"error":"failed"}\n`);
    ...
    expect(callbacks.onPartial).toHaveBeenCalledWith({ progress, partial: result });
