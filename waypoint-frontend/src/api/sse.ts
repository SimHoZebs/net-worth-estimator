...
export class SSETransportError extends Error {
  constructor(cause: unknown) {
    super('The server-sent event stream could not be read. Check the connection and retry.', { cause });
    this.name = 'SSETransportError';
  }
}

type StreamSource = ReadableStream<Uint8Array> | AsyncIterable<Uint8Array | string>;
...
function dispatchEvent(event: ParsedSSEEvent, callbacks: SSEParserCallbacks): void {
  callbacks.onEvent?.(event);
  if (event.event === 'progress') callbacks.onProgress?.(eventPayload<ProgressSSEData>(event.data));
  if (event.event === 'partial') callbacks.onPartial?.(eventPayload<PartialSSEData>(event.data));
  if (event.event === 'result') callbacks.onResult?.(eventPayload<ResultSSEData>(event.data));
  if (event.event === 'error') callbacks.onError?.(eventPayload<ErrorSSEData>(event.data));
}

function parseBlock(block: string, callbacks: SSEParserCallbacks): ParsedSSEEvent | Error | null {
  ...
  if (!isRecord(parsed)) return new SSEParseError(new Error('SSE data must be a JSON object.'));
  const event: ParsedSSEEvent = { event: eventName, data: parsed, rawData, ...(id === undefined ? {} : { id }) };
  return event;
}
  ...
  feed(chunk: Uint8Array | string): ParsedSSEEvent[] {
    ...
    const consume = (): boolean => {
      ...
      if (parsed instanceof Error) {
        this.failure = parsed instanceof SSEParseError ? parsed : new SSEParseError(parsed);
...
async function consumeReader(reader: ReadableStreamDefaultReader<Uint8Array>, parser: SSEParser, signal?: AbortSignal): Promise<SSEParseError | SSETransportError | null> {
  try {
    while (true) {
      if (signal?.aborted) return new SSETransportError(new DOMException('The event stream was aborted.', 'AbortError'));
    ...
    return cause instanceof SSEParseError ? cause : new SSETransportError(cause);
  }
  const finished = parser.finish();
  return finished instanceof Error ? finished : null;
}

async function consumeAsyncIterable(source: AsyncIterable<Uint8Array | string>, parser: SSEParser, signal?: AbortSignal): Promise<SSEParseError | SSETransportError | null> {
  try {
...
export async function parseSSE(input: SSEInput, callbacks: SSEParserCallbacks = {}, signal?: AbortSignal): Promise<SSEParseError | SSETransportError | null> {
  ...
  if (typeof Response !== 'undefined' && input instanceof Response) {
    if (!input.body) return new SSETransportError(new Error('The event response has no body.'));
  ...
  if ('getReader' in input && typeof input.getReader === 'function') return consumeReader(input.getReader(), parser, signal);
  return consumeAsyncIterable(input as AsyncIterable<Uint8Array | string>, parser, signal);
}
