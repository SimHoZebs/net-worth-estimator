...
import {
	EvidenceDialog,
	type EvidenceTarget,
} from "./components/EvidenceDialog.tsx";
import { type EditorTarget, PlanEditor } from "./components/PlanEditor.tsx";
	...
	return (
		<div className="app-shell">
						...
						{pages
							...
							.map((item) => (
								<li key={item.id}>
									<a
										className={
											page === item.id ? "nav-link active" : "nav-link"
										}
										href={`#${item.id}`}
									...
									>
										...
										{item.id === "compare" && changes.length > 0 && (
				...
				<div className="sidebar-bottom">
					<div className="local-card">
						...
						<button
							...
							onClick={() => setEvidence({ kind: "method" })}
						>
				...
				<header className="topbar">
					...
					<div className="topbar-status">
						...
						{loading ? (
							...
							<span
								className={`saved-indicator ${changes.length ? "draft-indicator" : ""}`}
							>
								{changes.length ? <GitBranch size={14} /> : <Check size={14} />}
								{changes.length
									? "Temporary version"
									: serverMode
										? "Saved server plan"
				...
				<main id="main-content" className="main-content">
					...
					{state.error && (
						<ErrorNotice
							message={state.error}
							action={
								state.error.includes("stale")
									? "Discard draft and load latest"
									: state.error.includes("another tab")
										? "Reload latest saved plan"
										: serverMode
					...
					{projection.base === null ? (
						<ProjectionLoading onRetry={projection.retryProjection} />
					) : projection.base instanceof Error ? (
						<ErrorNotice
							message={projection.base.message}
							action={
								serverMode ? "Retry server calculation" : "Inspect assumptions"
							}
							onAction={
								serverMode
									? projection.retryProjection
									: () => setEditor({ kind: "assumptions" })
							}
						/>
					) : (
						<>
							...
							{page === "outlook" && (
								<Outlook
									...
									onEvidence={() => setEvidence({ kind: "position" })}
									onFailure={() => setEvidence({ kind: "failure" })}
									onAccount={(account) =>
										setEvidence({ kind: "account", id: account.id })
									}
									onPlan={() => navigate("plan")}
									onGoals={() => navigate("goals")}
									onGoal={(id) => setEvidence({ kind: "goal", id })}
									onTiming={() => setEvidence({ kind: "timing" })}
									onAssumptions={() => setEditor({ kind: "assumptions" })}
								/>
							)}
							{page === "plan" && (
								<PlanPage
									...
									onAccount={(id) => setEvidence({ kind: "account", id })}
								/>
							)}
							{page === "goals" && (
								<GoalsPage
									...
									projection={projection.base}
									range={projection.range}
									onEdit={(item) => setEditor({ kind: "goal", item })}
									onUpdate={state.updatePlan}
									onEvidence={(id) => setEvidence({ kind: "goal", id })}
								/>
							)}
							{page === "compare" && !savedProjection && (
								<ProjectionLoading
									label="Loading the saved server comparison"
									onRetry={retrySavedProjection}
								/>
							)}
							{page === "compare" && savedProjection instanceof Error && (
								<ErrorNotice
							...
							{page === "compare" &&
								savedProjection &&
								!(savedProjection instanceof Error) && (
									<ComparePage
						...
						<div>
							...
							<span>
								...
								<small>
									...
									{changes.length === 1 ? "change" : "changes"} · saved{" "}
			...
			{discardOpen && (
				...
				>
					<p>
						Your {changes.length} unsaved{" "}
						{changes.length === 1 ? "change will" : "changes will"} be removed.
