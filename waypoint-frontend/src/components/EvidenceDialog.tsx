import { ArrowRight, Info, Pencil, TriangleAlert } from "lucide-react";
import { dateLabel, money, percent, shiftDate } from "../domain/format.ts";
...
export function EvidenceDialog({
	...
	const edit = (target: EditorTarget) => {
		onClose();
		onEdit(target);
	};
	...
	if (target.kind === "account") {
		const account = plan.accounts.find((a) => a.id === target.id);
	...
	if (target.kind === "failure") {
		...
		const movement = plan.movements.find((m) => m.id === failure.movementId);
		const account = plan.accounts.find((a) => a.id === failure.fromId);
		const destination = plan.accounts.find((a) => a.id === failure.toId);
		const bindingTypes = failure.constraintTypes ?? [];
		const sourceFloorBinding =
			bindingTypes.includes("source-floor") ||
			failure.constraint === "Protected account balance";
		const destinationCeilingBinding =
			bindingTypes.includes("destination-ceiling") ||
		...
		return (
			...
			>
				...
				<dl className="detail-list">
					<div>
						<dt>Shortfall</dt>
						<dd>{money(failure.requested - failure.realized)}</dd>
					</div>
					<div>
						<dt>Funding account</dt>
						<dd>{account?.name ?? "External income"}</dd>
					</div>
					<div>
						<dt>Funding evidence</dt>
						<dd>
							{failure.available === null
								? (failure.constraint ?? "Funded amount unavailable")
								: failure.available === Infinity
									? "External funding"
									: typeof failure.available === "number" &&
											Number.isFinite(failure.available)
					...
					<div>
						...
						<dd>{failure.constraint}</dd>
					</div>
					{sourceFloorBinding && (
						<div>
							<dt>Protected balance</dt>
							<dd>{money(account?.floor ?? 0)}</dd>
						</div>
					)}
					{destinationCeilingBinding && (
						<div>
							<dt>Destination ceiling</dt>
							<dd>{money(destination?.ceiling ?? 0)}</dd>
						</div>
					)}
					{!sourceFloorBinding && !destinationCeilingBinding && (
						<div>
							<dt>Binding constraint</dt>
							<dd>{failure.constraint}</dd>
						</div>
				...
				<div className="inline-notice amber">
					<Info size={19} />
					<span>
						The unpaid amount is not automatically borrowed or rescheduled.
						Later balances include only the realized movement, so the projected
				...
				{movement && (
					<div className="modal-actions">
						<button
							...
							onClick={() => edit({ kind: "movement", item: movement })}
						>
	...
	if (target.kind === "goal") {
		const result = projection.goals.find((g) => g.goal.id === target.id);
		...
		return (
			<Modal title={result.goal.name} eyebrow="Goal evidence" onClose={onClose}>
					...
					<div>
						...
						<dd>
							{result.goal.kind === "net-worth"
								? "Household net worth"
								: plan.accounts.find((a) => a.id === result.goal.accountId)
										?.name}
						</dd>
					</div>
					<div>
						<dt>Target</dt>
						<dd>{money(result.goal.target)}</dd>
					</div>
					<div>
						<dt>At the start</dt>
						<dd>{money(result.current)}</dd>
					</div>
					<div>
						<dt>At the horizon</dt>
						<dd>{money(result.final)}</dd>
					</div>
					<div>
						<dt>Still met at horizon</dt>
						<dd>{result.final >= result.goal.target ? "Yes" : "No"}</dd>
					</div>
					{range && (
						<div>
							<dt>Scenarios reaching the target</dt>
							<dd>
								{percent(range.goalSuccess[result.goal.id] ?? 0)} of{" "}
								{range.count}
							</dd>
						</div>
					)}
				...
				<p className="section-note">
					The first crossing of a threshold is a milestone. It does not
					establish sustained spending coverage, retirement readiness, or future
					certainty.
				</p>
				<div className="modal-actions">
					<button
						type="button"
						className="button primary"
						onClick={() => edit({ kind: "goal", item: result.goal })}
					>
						<Pencil size={16} />
						Edit goal
					</button>
	...
	if (target.kind === "timing") {
		const end = shiftDate({ date: plan.startDate, days: 30 });
		const events = projection.movements.filter((m) => m.date <= end);
		return (
			<Modal
			...
			<div className="method-steps">
				<div>
					...
					<div>
						...
						<p>
							Recorded balances and clearly marked estimates establish today’s
							position.
						</p>
					</div>
				</div>
				<div>
					<span>02</span>
					<div>
						<h3>Follow the plan forward</h3>
						<p>
							Dated income, spending, transfers and account rates produce a base
							case. Optional ranges vary investment returns across 400
							scenarios.
						</p>
					</div>
				</div>
				<div>
					<span>03</span>
					<div>
						<h3>Try a change, then decide</h3>
						<p>
							Edit a temporary version, compare the outcome, and explicitly save
							or discard. Drafts stay in this browser between visits.
						</p>
					</div>
				</div>
			</div>
