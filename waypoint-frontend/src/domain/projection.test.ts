...
import { changesBetween, netWorth, validatePlan, type Plan } from './model.ts';
import { calculateRange, currentNetWorth, horizonDate, occurrences, project, quantile } from './projection.ts';
import { payEvidence } from './evidence.ts';

const blank = (): Plan => ({ ...structuredClone(examplePlan), accounts: [{ ...examplePlan.accounts[0]!, balance: 10000, annualReturn: 0, floor: 0 }], movements: [], goals: [] });
...
describe('validated plans', () => {
  ...
  it('distinguishes added, modified, removed and excluded records', () => {
    ...
    expect(changesBetween({ saved: examplePlan, current: next }).map((c) => c.kind)).toEqual(['Modified', 'Removed', 'Added']);
  });
});
...
describe('dated projections', () => {
  it('keeps empty zero-rate plans unchanged and carries their starting position', () => {
    ...
    expect(currentNetWorth({ projection: result, plan: { ...plan, accounts: [{ ...plan.accounts[0]!, balance: 20000 }] } })).toBe(10000);
  ...
  it('respects protected balances and records the exact shortfall', () => {
    ...
    plan.movements = [{ ...examplePlan.movements[1]!, frequency: 'once', startDate: '2026-10-01', amount: 12000 }];
    const result = project({ plan, years: 10 });
    expect(result.firstFailure).toMatchObject({ date: '2026-10-01', requested: 12000, realized: 7500, constraint: 'Protected account balance' });
  ...
  it('does not create money in internal transfers', () => {
    ...
    plan.movements = [{ ...examplePlan.movements[3]!, toId: 'other', amount: 1500, annualIncrease: 0 }];
    const result = project({ plan, years: 1 });
    expect(result.points.at(-1)?.total).toBe(10000);
  ...
  it('stops incoming transfers at the destination ceiling', () => {
    const plan = blank();
    plan.accounts.push({ ...plan.accounts[0]!, id: 'other', balance: 0, ceiling: 100 });
    plan.movements = [{ ...examplePlan.movements[3]!, toId: 'other', frequency: 'once', amount: 1500, annualIncrease: 0 }];
    const result = project({ plan, years: 1 });
  ...
  it('stops debt payments at zero without reporting a spurious shortfall', () => {
    const plan = blank();
    plan.accounts.push({ ...plan.accounts[0]!, id: 'mortgage', kind: 'debt', balance: -500, ceiling: 0 });
    plan.movements = [{ ...examplePlan.movements[2]!, amount: 700, annualIncrease: 0 }];
    const result = project({ plan, years: 1 });
  ...
  it('clamps month-end recurrences and leap-year horizons', () => {
    const plan = blank();
    plan.movements = [{ ...examplePlan.movements[0]!, startDate: '2027-01-31' }];
    expect(occurrences({ plan, endDate: '2027-03-31' }).map((e) => e.date)).toEqual(['2027-01-31', '2027-02-28', '2027-03-31']);
  ...
  it('increases amounts on the actual anniversary rather than a rounded year length', () => {
    const plan = blank();
    plan.movements = [{ ...examplePlan.movements[0]!, amount: 1000, annualIncrease: 10, startDate: '2026-10-01' }];
    const events = occurrences({ plan, endDate: '2027-10-01' });
    expect(events.find((event) => event.date === '2027-09-01')?.amount).toBe(1000);
    expect(events.find((event) => event.date === '2027-10-01')?.amount).toBe(1100);
  });
  it('finds the earliest base-case failure and goal crossing', () => {
    ...
    expect(result.goals.find((g) => g.goal.id === 'cash-buffer')?.firstDate).toBe(examplePlan.startDate);
    expect(result.goals.find((g) => g.goal.id === 'million')?.firstDate).toBeTruthy();
  });
});

describe('scenario ranges and inference', () => {
  it('uses a percentile distribution rather than the band midpoint', () => {
    expect(quantile({ values: [1, 2, 3, 50, 100], fraction: .5 })).toBe(3);
  });
  it('is repeatable, keeps boundaries ordered, and collapses with zero variability', () => {
    ...
    for (const point of first.points) expect(point.lower === point.median && point.median === point.upper).toBe(true);
  });
  it('varies investment assumptions and retains the fixed starting position', () => {
    ...
    expect(result.points.at(-1)!.lower).toBeLessThan(result.points.at(-1)!.median);
    expect(result.points.at(-1)!.median).toBeLessThan(result.points.at(-1)!.upper);
  });
  it('keeps pay inference separate from planned income and excludes outliers', () => {
    const plan = structuredClone(examplePlan);
    plan.movements.push({ ...plan.movements[6]!, id: 'outlier', amount: 100000, startDate: '2026-05-28' });
