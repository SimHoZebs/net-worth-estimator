...
export function occurrences({
	...
	for (const movement of plan.movements) {
		...
		if (
			movement.fromId &&
			!plan.accounts.some(
				(account) => account.id === movement.fromId && account.enabled,
			)
		)
			continue;
		if (
			movement.toId &&
			!plan.accounts.some(
				(account) => account.id === movement.toId && account.enabled,
			)
		)
			continue;
		...
		const append = (date: string) => {
			if (
				...
				date > endDate ||
				(movement.endDate && date > movement.endDate)
			)
				return;
			const yearDifference = Number(date.slice(0, 4)) - firstYear;
			const anniversary = monthDate({
				year: firstYear + yearDifference,
				month: firstMonth,
			...
			result.push({
				...
				amount:
					movement.amount *
					(1 + movement.annualIncrease / 100) ** Math.max(0, elapsedYears),
			});
		};
		if (movement.frequency === "once") {
			append(movement.startDate);
			continue;
		}
		for (
			let year = Math.max(firstYear, Number(plan.startDate.slice(0, 4)));
			year <= endYear;
			year++
		) {
...
export function project({
	...
	const balances = Object.fromEntries(
		plan.accounts.filter((a) => a.enabled).map((a) => [a.id, a.balance]),
	);
	const points: Point[] = [];
	const movements: MovementResult[] = [];
	const goals: GoalResult[] = plan.goals
		.filter((g) => g.enabled)
		.map((goal) => ({
			goal,
			firstDate: null,
			current: goalValue({ goal, balances }),
			final: 0,
		}));
	...
	for (const date of dates) {
		...
		for (const account of plan.accounts.filter((item) => item.enabled)) {
			const shock =
				account.kind === "investment"
					? (annualShocks[Number(date.slice(0, 4)) - firstYear] ?? 0)
					: 0;
			const rate = Math.max(-50, Math.min(50, account.annualReturn + shock));
			balances[account.id] =
				(balances[account.id] ?? 0) * (1 + rate / 100) ** (days / 365.25);
		}
		for (const event of byDate.get(date) ?? []) {
			...
			const available = source
				? Math.max(0, (balances[source.id] ?? 0) - source.floor)
				: Infinity;
			const debtRemaining =
				destination?.kind === "debt"
					? Math.max(0, -(balances[destination.id] ?? 0))
					: Infinity;
			const headroom =
				destination && destination.ceiling !== null
					? Math.max(0, destination.ceiling - (balances[destination.id] ?? 0))
					: Infinity;
			...
			if (destination)
				balances[destination.id] = (balances[destination.id] ?? 0) + realized;
			const constraint =
				realized + 0.01 < requested
					? available < requested && available <= headroom
						? "Protected account balance"
						: "Destination account ceiling"
					: null;
			const accountDeltas = [
				...(source ? [{ accountId: source.id, delta: -realized }] : []),
				...(destination
					? [{ accountId: destination.id, delta: realized }]
					: []),
			];
		...
		if (markers.has(date))
			points.push({
				date,
				total: sum(Object.values(balances)),
				balances: { ...balances },
			});
		for (const result of goals) {
			const value = goalValue({ goal: result.goal, balances });
			if (value >= result.goal.target && !result.firstDate)
	...
	return {
		...
		outflows,
		transfers,
		startDateHasCheckpoint: plan.accounts.some(
			(account) =>
				account.enabled &&
				account.observedOn === plan.startDate &&
...
export function calculateRange({
	...
	const goalSuccess = Object.fromEntries(
		plan.goals.filter((g) => g.enabled).map((g) => [g.id, 0]),
	);
	let dates: string[] = [];
	let failures = 0;
	for (let run = 0; run < count; run++) {
		const annualShocks = Array.from(
			{ length: years + 1 },
			() =>
		...
		result.points.forEach((point, index) => {
			(distributions[index] ??= []).push(point.total);
		});
		for (const goal of result.goals)
			if (goal.firstDate)
				goalSuccess[goal.goal.id] =
					(goalSuccess[goal.goal.id] ?? 0) + 1 / count;
		if (result.firstFailure) failures++;
		if (run % 20 === 0) onProgress?.((run + 1) / count);
	}
	return {
		points: distributions.map((values, index) => ({
			date: dates[index] ?? plan.startDate,
			lower: quantile({ values, fraction: 0.1 }),
			median: quantile({ values, fraction: 0.5 }),
			upper: quantile({ values, fraction: 0.9 }),
		})),
		count,
		goalSuccess,
		failureShare: failures / count,
	};
}
