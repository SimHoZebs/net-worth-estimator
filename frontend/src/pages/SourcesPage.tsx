		...
		<>
			...
			<div className="sources-grid">
				<section className="panel source-health">
					...
					<dl className="detail-list">
						...
						<div>
							<dt>{serverMode ? "Display start" : "Starting position"}</dt>
							<dd>
						...
						{serverMode && (
							<div>
								<dt>Server postings</dt>
								<dd>{serverPostingCount}</dd>
							</div>
						)}
						<div>
							<dt>{serverMode ? "Display validation" : "Validation"}</dt>
							<dd className="inline-success">
								<Check size={15} />
						...
						<div>
							<dt>
								{serverMode ? "Display revision date" : "Last local save"}
							</dt>
							<dd>{dateLabel(workspace.saved.updatedAt, true)}</dd>
						</div>
						<div>
							<dt>{serverMode ? "Display revision" : "Saved revision"}</dt>
							<dd>{workspace.saved.revision}</dd>
						</div>
						<div>
							<dt>Source access</dt>
							<dd>{sourceAccess}</dd>
					...
					<p className="section-note">
						Validation checks structure and references. It does not
						independently verify balances, bank provenance, or financial
						assumptions.
					</p>
				</section>
				<section className="panel portability">
					...
					<button
						...
						onClick={exportPlan}
						disabled={serverMode && !serverDocument}
					>
						<Download size={21} />
						<span>
							<strong>
								{serverMode
									? "Export server model"
									: `Export ${workspace.draft ? "temporary" : "saved"} plan`}
							</strong>
							<small>
								{serverMode
					...
					<button
						...
						disabled={
							reading ||
							importing ||
							(serverMode && (readOnly || !onImportServerDocument))
						}
					>
						<FileUp size={21} />
						<span>
			...
			<section className="panel pay-evidence">
				...
				<div className="inline-notice">
					<Info size={18} />
					<span>
						These inflows may include non-payroll income. This inference does
						not establish gross salary or bank provenance and never changes{" "}
						{serverMode ? "the canonical server model" : "your plan"}. Fewer
						than six comparable records limit confidence.
					</span>
				</div>
			</section>
			{planCandidate && (
				...
				>
					...
					<div className="modal-actions">
						...
						<button
							...
							disabled={Boolean(workspace.draft)}
							onClick={applyPlan}
						>
							Replace workspace
						</button>
			...
			{serverCandidate && (
				...
				>
					<div className="import-summary">
						<FileJson size={30} />
						<h3>
							{serverCandidate.sourcePath.split("/").filter(Boolean).at(-1) ||
								"Waypoint model"}
						</h3>
						<p>
							{serverCandidate.accounts.length} accounts ·{" "}
							{serverCandidate.postings.length} postings ·{" "}
							{serverCandidate.checkpoints.length} checkpoints
						</p>
						<p>
							{serverCandidate.evaluations.financialIndependence.length +
								serverCandidate.evaluations.netWorthThreshold.length +
								serverCandidate.evaluations.postingFulfillment.length}{" "}
							evaluations
						</p>
					</div>
					<div className="inline-notice">
